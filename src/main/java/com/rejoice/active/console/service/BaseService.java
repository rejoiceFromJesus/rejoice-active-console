...
public abstract class BaseService<T> {
	...
	public List<T> queryListByWhereOrderByClause(T t, String orderByClause)
			{
		// 声明一个example
		Example example = new Example(this.clazz);
	...
	/**
	 * 根据ids批量删除,delete logically by update flag=0
	 * 
	 * @param list
	 */
	public void deleteByIdsLogically(List<String> list) {
		// 设置条件
		Example example = new Example(clazz);
		example.createCriteria().andIn("id", list);
		T record;
		try {
			...
			Field flag = ReflectionUtils.findField(clazz, "flag");
			flag.setAccessible(true);
			flag.set(record, 0);
		} catch (Exception e) {
			throw new InternalServerException("new instance by reflection fails,class=["+clazz+"]");
		}
		// 根据条件删除
	...
	/**
	 * 
	 * add conditions with likes, ins ,equal and default flag=1
	 * @param t
	 * @param criteria
	 * void
	 */
	protected void equalOrLikeOrIn(T t, Criteria criteria) {
		//default conditions: flag=1
		setFlag(t);
		//handle like query
	...
	*/
	...
	/**
	 * setFlag(这里用一句话描述这个方法的作用)
	 * (这里描述这个方法适用条件 – 可选)
	 * @param t
	 * void
	*/
	protected void setFlag(T t) {
		try {
			Field flag = ReflectionUtils.findField(t.getClass(), "flag");
			if(null == flag) return;
			flag.setAccessible(true);
			Object object = flag.get(t);
			if (null == object) {
				flag.set(t, 1);
			}
		} catch (Exception e) {
